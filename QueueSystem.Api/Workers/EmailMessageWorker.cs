...
{
    public class EmailMessageWorker : BackgroundService
    {
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<EmailMessageWorker> _logger;
        private readonly int _retryIntervalSeconds;
        private readonly int _maxRetries;
        private readonly int _maxDurationHours;

        public EmailMessageWorker(IServiceProvider serviceProvider, ILogger<EmailMessageWorker> logger, IOptions<EmailSenderSettings> options)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var status = await settingsRepo.GetByKeyAsync("EmailSendingStatus");
                    if (status?.Value == "Paused")
                    {
                        _logger.LogInformation("Email sending is paused.");
                ...
                {
                    _logger.LogError(ex, "Error in EmailMessageWorker loop");
                }
                await Task.Delay(TimeSpan.FromSeconds(_retryIntervalSeconds), stoppingToken);
            }
        }
